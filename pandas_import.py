...
for i in range(len(data_lst)):
    data_lst[i] = data_lst[i][pd.to_numeric(data_lst[i]['value'], errors='coerce').notnull()]
    data_lst[i]['value'] = data_lst[i]['value'].astype(float)
    data_lst[i].rename(columns={'value': str(files_lst[i].replace('_small.csv', ''))}, inplace=True)
